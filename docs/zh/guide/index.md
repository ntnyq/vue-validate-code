...
::: warning 安全提示
`vue-validate-code` 完全在浏览器中生成并校验验证码。SVG 渲染器会将字符暴露为 DOM 文本，客户端校验始终可以被绕过。它只能作为视觉交互辅助，不能用于 CAPTCHA、身份验证或防滥用。安全敏感场景必须使用服务端生成并校验的挑战。
:::
